...
class ExpenseDB {
  ...
  initialize() async {
    ...
    return openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute("CREATE TABLE ExpensesPerMonth (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT, price REAL)");
      }
    );
  }
  ...
  Future<List<Month>> getPerMonth() async {
    Database db = await database;
    List<Map> query = await db.rawQuery("SELECT * FROM ExpensesPerMonth ORDER BY date DESC");
    var result = List<Month>();
    query.forEach((r) => result.add(Month(r["id"], DateTime.parse(r["date"]), r["price"])));
    return result;
  }
  ...
  Future<void> addExpensePerMonth(DateTime date, double price) async {
    ...
    List<Map> query = await db.rawQuery("SELECT price FROM ExpensesPerMonth WHERE date = ?;", ["$month"]);
    if (query.isEmpty) {
      db.rawInsert("INSERT INTO ExpensesPerMonth (date, price) VALUES (?, ?)", ["$month", "$price"]);
    } else {
      db.rawUpdate("UPDATE ExpensesPerMonth SET price = price + ? WHERE date = ?", ["$price", "$month"]);
    }
  }
